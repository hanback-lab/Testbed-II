...
## 나만의 TestBed Controller 
'3-3. GUI' 내용을 바탕으로 TestBed 제어 프로그램을 작성해 보겠습니다. 작성할 프로그램에 필수로 적용되어야 하는 내용은 다음과 같습니다. 
>- 센서 데이터 
>    - 표기할 센서 목록
>        - Light(Room), Temperature(Room), Humidity(Room), Dust
>    - 센서 데이터 갱신 주기는 1초 
>- 액츄에이터 
>    - Lamp 
>        - 동작 시킬 Lamp 위치 선택 후 On 또는 Off 버튼에 따른 제어 
>    - Fan    
>        - Room의 Fan을 On 또는 Off 버튼 제어에 따른 제어 
...
새로운 UI 생성을 진행합니다. 생성할 UI는 "Templates" 에서 "Main Window" 를 선택하면 기본적인 창 구성이 가능합니다. 다음은 TestBed Controller UI 구성입니다. 

![TestBed Controller UI](res/testbed_controller_ui.png)

앞의 그림에 표기된 UI 에는 눈으로 확인되지 않는 Label을 포함하여 10개의 Label과 4개의 PushButton, 1개의 ComboBox로 이루어집니다. 각각의 위젯의 기본값 설정은 다음과 같습니다. 

**여기서 Geometry 는 꼭 아래의 내용을 따르지 않아도 되며 원하는대로 구성해도 관계 없습니다.**

| Class | Object Name | Description | Geometry | Default Value | 
|:-------|:------|:------|:------|:------|
| QLabel | LightLbl | 문자 표기 | [(20,20),41x41] | Light |
| QLabel | LightValueLbl | 센서 데이터 표기 | [(110,20),81x41] | --- |
| QLabel | DustLbl | 문자 표기 | [(200,20),41x41] | Dust |
| QLabel | DustValueLbl | 센서 데이터 표기 | [(270,20),111x41] | --- |
| QLabel | TempLbl | 문자 표기 | [(20,70),71x41] | Temperature |
| QLabel | TempValueLbl | 센서 데이터 표기 | [(110,70),81x41] | --- |
| QLabel | HumiLbl | 문자 표기 | [(200,70),71x41] | Humidity |
| QLabel | HumiValueLbl | 센서 데이터 표기 | [(270,70),81x41] | --- |
| QLabel | LampLbl | 문자 표기 | [(20,120),41x41] | Lamp |
| QComboBox | LampCB | 제어 위치 선택 | [(90,130),80x24] | --- |
| QPushButton | LampOnBtn | 램프 켜기 | [(180,130),79x24] | ON |
| QPushButton | LampOffBtn | 램프 끄기 | [(270,130),79x24] | OFF |
| QLabel | FanLbl | 문자 표기 | [(20,170),41x41] | Fan |
| QPushButton | fOnBtn | 도어락 열기 | [(90,180),79x24] | ON |
| QPushButton | fOffBtn | 도어락 닫기 | [(180,180),79x24] | OFF |
...
### TestBed Controller 어플리케이션  
UI 파일을 기반으로 동작 코드를 구현합니다. 센서 데이터 자동 갱신을 위해 타이머를 구성하고 램프 및 도어락 제어를 위한 버튼의 동작을 정의합니다. 램프의 경우 위치 선택을 위해 ComboBox를 구성하고 ComboBox의 아이템을 ["entrance","room","kitchen"] 으로 설정합니다. 
...
from testbed.sensor import Light, Tphg, Dust
from testbed.actuator import LampGroup, Fan
...
class Window(QMainWindow):
    def __init__(self):
        ...
        self.__light = Light('ROOM')
        self.__tphg = Tphg('ROOM')
        self.__dust = Dust() 
        self.__lamp = LampGroup() 
        self.__fan = Fan('ROOM') 
        self.ui.LampCB.addItems(["entrance","room","kitchen"])
        self.ui.fOnBtn.clicked.connect(self.fan_clicked)
        self.ui.fOffBtn.clicked.connect(self.fan_clicked)
    ...
    def sensor_update(self):
        self.ui.LightValueLbl.setText(str(self.__light.read()['state']))
        temp = self.__tphg.read()
        self.ui.TempValueLbl.setText(str(temp['temp']))
        self.ui.HumiValueLbl.setText(str(temp['humi']))
        temp = self.__dust.read()
        self.ui.DustValueLbl.setText("1.0 : "+str(temp['pm1_0'])+", 2.5 : "+str(temp['pm2_5'])+", 10 : "+str(temp['pm10']))
...
![TestBed Controller](res/testbed_controller.png)
...
### Slack 을 활용한 고온 경고 
TestBed의 Tphg 센서의 온도 데이터를 활용하여 설정한 온도 이상인 경우 알림 메시지를 전달하도록 코드를 구현하면 다음과 같이 구현할 수 있습니다. 온도 경고를 위한 임계치는 테스트 환경에 따라 변경하여 활용하시기 바랍니다. 
...
- 메인 윈도우 
    ...
    - 알림 메시지 선택 
    - "SET" 버튼을 통해 설정을 진행하고 설정된 정보에 따라 TestBed 의 온도 센서 데이터와 비교 및 메시지 전달
        - 버튼 입력후 UNSET으로 변경
    - "UNSET" 버튼을 통해 동작 중지 
</details>