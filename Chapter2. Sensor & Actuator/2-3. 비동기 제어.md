...
TestBed의 현관 구역에 장착된 TPHG 센서와 Light 센서를 활용한 센서 데이터 수집입니다. 우선 순차적으로 수집하여 화면에 출력하는 방식의 코드입니다. 
...
def monitor_tphg():
    data = tphg.read()
    print(f" Temp: {data['temp']}°C / Humidity: {data['humi']}% ")
    time.sleep(5)

def monitor_light():
    data = light.read()
    print(f" Lux: {data['state']}")
    time.sleep(2)
...
```python
import asyncio
from testbed.sensors import Light, Tphg  
