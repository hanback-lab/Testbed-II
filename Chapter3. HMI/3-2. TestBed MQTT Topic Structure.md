...
사용자는 PC에서 SET으로 “요청”만 하고, EdgeAI는 이를 받아 정책/권한/충돌 처리를 수행한 뒤 Auto Controller에 ACT로 “실행 명령”을 전달합니다.

Auto Controller는 실제 하드웨어를 제어한 결과와 센서 데이터를 STATE로 발행하여, PC/EdgeAI가 같은 방식으로 모니터링할 수 있게 합니다.

즉, SET → ACT → STATE는 “요청–중재–실행–피드백”의 파이프라인이며, 여러 사용자가 동시에 접속하는 환경에서도 충돌 방지, 로깅/추적, 보안(익명 허용 범위 제한), 확장성을 확보하기 위한 구조입니다.

마지막으로 AVAILABILITY 토픽은 Auto Controller의 온라인/오프라인 상태를 제공하여, EdgeAI가 장치 연결 상태를 빠르게 판단하고 장애 상황을 사용자에게 안내할 수 있도록 돕습니다.
...
토픽의 기본 구조는 앞의 설명과 같이 장비의 고유 식별자와 장비 내부 구역정보, 장치의 이름, 제어 명령 혹은 상태가 포함됩니다. 이를 세분화 하면 TestBed에서 MQTT 통신을 활용하는 3가지 기기(PC, EdgeAI, Auto제어기)에서 활용 하는 정보가 각각 다르며, 사용자에 따른 토픽 구독 가능 여부도 구분되어 있습니다. 
...
>- PC Topic 
>    - Publish Topic List
>        - TB/+/+/+/SET : EdgeAI에 장치 제어 요청 토픽, 익명 사용자를 허용 
...
>        - TB/+/+/AVAILABILITY : Auto 제어기의 연결 상태 토픽 
>    - Subscribe Topic List 
>        - TB/+/+/+/ACT : EdgeAI에서 발행한 장치 제어 요청 토픽을 구독 
...
## 토픽 발행을 활용한 Lamp 제어 
TestBed에서는 사용자가 직접 하드웨어를 제어하지 않습니다. 사용자는 단지 “요청(SET)” 을 보내고, EdgeAI가 이를 받아 정책·권한·충돌 처리 후 Auto Controller에게 “실행 명령(ACT)” 을 전달합니다.
