...
**단순 비밀번호만 사용하는 시스템은 유출 시 매우 취약합니다.** 이를 보완하기 위해 2FA를 활용하는것이 하나의 보안 방안이 될 수 있습니다. 만약 공격자가 비밀번호를 알고 있는 상황이라 하여도 2차 인증 없이는 접근 불가능하기 때문입니다. 
...
## 보안이 강화된 나만의 TestBed Controller 
이번에는 '3-4 실시간 모니터링 시스템 구현' 에서 제작한 TestBed Controller 의 기능을 강화 해보도록 하겠습니다. 
기존 프로그램은 사용자 인증, 그리고 OTP 와 같은 기능을 활용하고 있지 않으며 프로그램을 실행하면 연결되어 있는 장치를 바로 제어하는 형태였습니다. 
앞서 제작한 OTP 인증을 통해 본인의 스마트폰으로 인증 절차를 거친 사용자만 TestBed를 제어할 수 있도록 구성합니다. 동작 구조는 다음과 같습니다. 

- OTP 를 통한 사용자 인증 
    - 비밀키 생성은 최초 프로그램 실행 시 생성 되어 파일로 저장 
    - 비밀키 기반으로 QR Code 생성 및 화면 출력 
    - QR Code가 이미 생성되어 있다면, 이전 내용을 재사용 
    - OTP 인증이 완료되면 TestBed Controller 윈도우를 출력하여 제어 

GUI 코드는 앞서 작성한 TestBed Controller 를 기반으로 합니다. TestBed Controller 에서 수정되는 내용은 다음과 같습니다. 
...
from testbed.sensor import Pir, Light, Tphg, Dust
from testbed.actuator import Lamp, DoorLock
...
## 연습문제 
TestBed Controller 의 보안기능을 다음 조건을 만족하도록 수정해 보세요. 
...
- OTP 윈도우 
    ...
    - OTP 인증 절차가 완료되면 TestBed Controller 윈도우 출력 
- TestBed Controller 윈도우 
    - 센서 데이터 출력 
        - Pir, Light, Temperature, Humidity, Dust
    - 액츄에이터 제어 
        - Lamp, DoorLock, Curtain 
</details>